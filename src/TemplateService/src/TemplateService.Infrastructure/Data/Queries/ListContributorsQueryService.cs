...
using TemplateService.UseCases.Contributors;
using TemplateService.UseCases.Contributors.List;

namespace TemplateService.Infrastructure.Data.Queries;
...
public class ListContributorsQueryService : IListContributorsQueryService
{
    ...
    public async Task<UseCases.PagedResult<ContributorDto>> ListAsync(int page, int perPage)
    {
        var items = await _db.Contributors
            .FromSqlRaw(
                "SELECT Id, Name, PhoneNumber_CountryCode, PhoneNumber_Number, PhoneNumber_Extension FROM Contributors") // don't fetch other big columns
            .OrderBy(c => c.Id)
            .Skip((page - 1) * perPage)
            .Take(perPage)
